...
def render_weather_widget(
    ...
    if forecast and len(forecast) > 0:
        for day_data in forecast[:6]:  # Show max 6 days
            ...
            temp_min_day = day_data.get("temperature_min", 0)
            temp_max_day = day_data.get("temperature_max", 0)
            day_weather_code = day_data.get("weather_code", 0)
            emoji = get_weather_emoji(day_weather_code, False)
            ...
            weekly_forecast_items.append(
                Col(
                    ...
                    children=[
                        ...
                        Text(value=f"{temp_max_day:.0f}°", size="sm", weight="medium"),
                        Text(value=f"{temp_min_day:.0f}°", size="xs", color="alpha-50"),
                    ],
                )
            )
